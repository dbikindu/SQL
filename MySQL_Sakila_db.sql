...
SELECT cu.first_name, cu.last_name, cu.email, ctry.country 
FROM customer cu
JOIN address a ON 
(cu.address_id = a.address_id)
JOIN city ct ON 
(a.city_id=ct.city_id)
JOIN country ctry ON 
(ct.country_id=ctry.country_id)
...
SELECT f.title, COUNT(f.title) AS 'Count_rented_movies'
FROM film f
JOIN inventory i 
...
SELECT cat.name AS 'Film Genre', SUM(p.amount) AS 'Gross Revenue'
FROM category cat
